          ...
          function replacePlaceholdersRecursivelyIn (string) {
            ...
            return replaceP(String(string), self.regex, function (match, index, gt) {
              ...
              if (resolvedValue == null) {
                var failedPromiseArgs = self.promiseStore[index].promiseArgsForFailure;
                var fnName = failedPromiseArgs[failedPromiseArgs.length - 1].name
                throw new Error(`Promise has no result: ${fnName} ${failedPromiseArgs}`)
              }
